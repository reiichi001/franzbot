	...
	if (isDirectMessage || GoatTriggers.includes(message.guild.id)) {
		...
		if (message.attachments.size > 0)  {
			...
			message.attachments.forEach(async attachment => {
				...
				if (isDirectMessage && attachment.name.match(/(output|dalamud|message|dalamudConfig|launcherConfigV3|dxdiag|event|SquirrelSetup|patcher).*\.(log|txt|json|evtx)/gui)) {
					// sane filesizes only
