...
module.exports = (client, member) =>
{
	...
	if ( ZuTriggers.includes(member.guild.id ) )
	{
		...
		member.guild.channels.cache.find(c => c.name === "welcome").send(`${member.user}`)
			.then(member.guild.channels.cache.find(c => c.name === "welcome").send(embedobj).catch(console.error));
